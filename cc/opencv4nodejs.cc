...
public:
    ...
    {
        std::string msg = "OpenCV Error: (" + std::string(err_msg) + ")" + " in " + std::string(func_name) + ", in file " + std::string(file_name) + ", line " + std::to_string(line) + ", status " + std::to_string(status);
    ...
    void init(v8::Local<v8::Object> target)
    {
        ...
        node::AddEnvironmentCleanupHook(v8::Local<v8::Object>, DeleteInstance, this);
        ...
        Core::Init(target);
        node::AddEnvironmentCleanupHook(target, DeleteInstance, this);
#ifdef HAVE_OPENCV_CALIB3D
...
NODE_MODULE_INIT()
{
    AddonData *data = new AddonData(target);
    Local<External> external = External::New(target, data);
    exports->Set(context,
               String::NewFromUtf8(target, "method", NewStringType::kNormal)
                  .ToLocalChecked(),
               FunctionTemplate::New(target, Method, external)
